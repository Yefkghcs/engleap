      ...
      <div className="max-w-7xl mx-auto px-4">
        <div className="flex items-center justify-between h-16">
          ...
          <div className="flex items-center gap-3 sm:gap-6 md:gap-8">
            ...
            {user ? <Link to="/profile" className={`flex items-center gap-2 text-sm font-medium transition-all duration-200 hover:text-primary ${isActive("/profile") ? "text-primary font-semibold" : "text-muted-foreground"}`} title={user.email}>
                <User className="h-4 w-4" />
                <span className="hidden lg:inline max-w-[150px] xl:max-w-[200px] truncate">{user.email}</span>
              </Link> : <Link to="/auth" className={`flex items-center gap-2 text-sm font-medium transition-all duration-200 hover:text-primary ${isActive("/auth") ? "text-primary font-semibold" : "text-muted-foreground"}`}>
                <User className="h-4 w-4" />
